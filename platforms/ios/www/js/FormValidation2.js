...
function init() {
   ...
   document.getElementById("input").onsubmit = validateForm;
...
function validateForm() {
   return (isNotEmpty("name", "Please enter your name!")
        && isNotEmpty("nationality", "Please enter your nationality!")
        && isValidEmail("email", "Enter a valid email!")
        && isLengthMinMax("password", "Enter a valid password!", 8,14)
        && verifyPassword("password", "pwVerified", "Different from the password!")
        && isSelected("community", "Please make a selection!"));
}
